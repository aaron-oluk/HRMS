            ...
            <tbody class="divide-y divide-slate-100">
                @forelse ($departments as $department)
                    <tr>
                        ...
                        <td class="px-4 py-3"><x-badge :color="$department->status === 'active' ? 'success' : 'neutral'">{{ ucfirst($department->status) }}</x-badge></td>
                        <td class="px-4 py-3 text-right">
                            @can('org.manage')
                                <a href="{{ route('departments.edit', $department) }}" class="text-emerald-600 hover:text-emerald-500">Edit</a>
                            @endcan
                        </td>
                    </tr>
                @empty
