            ...
            <tbody class="divide-y divide-slate-100">
                @forelse ($positions as $position)
                    <tr>
                        ...
                        <td class="px-4 py-3"><x-badge :color="$position->status === 'active' ? 'success' : 'neutral'">{{ ucfirst($position->status) }}</x-badge></td>
                        <td class="px-4 py-3 text-right">
                            @can('org.manage')
                                <a href="{{ route('positions.edit', $position) }}" class="text-emerald-600 hover:text-emerald-500">Edit</a>
                            @endcan
                        </td>
                    </tr>
                @empty
